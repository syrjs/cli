  ...
  if (_project.packageJSON && _project.packageJSON.devDependencies) {
    Object.keys(_project.packageJSON.devDependencies).forEach(function (key, index) {
      if (modules.indexOf(key) > -1) {
        if (_rc.dependencies && !_rc.dependencies.get(_project.packageJSON.version)[key]) {
          _rc.dependencies.add(key, _project.packageJSON.devDependencies[key], _project.packageJSON.version);
        }
        ret.push({
          module: key,
