...
var api = {
  init: function init(projectName) {
    ...
    var projectPath = _path2.default.join(process.cwd(), projectName);
    ...
    copyFixtures(fixtures, projectPath, function() {
      ...
      copyFixtures(
        [indexFixture],
        _path2.default.join(projectPath, 'src'),
        function() {
          // copy iOS project
          copyDir(
            _path2.default.join(projectPath, syrProjectPaths.ios),
            _path2.default.join(projectPath, 'ios')
          );
        }
      );
    });
  ...
  if (!_fs2.default.existsSync(projectPath)) {
    ...
    fixtures.forEach(function(meta) {
      _fs2.default.writeFileSync(
        _path2.default.join(projectPath, meta.name),
        meta.contents
      );
    });
    callback();
...
function copyDir(src, dest) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if (current.isDirectory()) {
      copyDir(
        _path2.default.join(src, files[i]),
        _path2.default.join(dest, files[i])
      );
    } else if (current.isSymbolicLink()) {
      var symlink = _fs2.default.readlinkSync(
        _path2.default.join(src, files[i])
      );
